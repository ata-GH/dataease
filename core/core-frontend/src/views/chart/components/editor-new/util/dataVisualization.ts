...
export const DEFAULT_DASHBOARD_STYLE_BASE = {
  ...
  resultMode: 'all', // 图表结果显示模式 all 图表 custom 仪表盘自定义
...
export const DEFAULT_CANVAS_STYLE_DATA_BASE = {
  ...
  refreshBrowserUnit: 'minute', // 仪表盘刷新时间带外 默认 分钟
  refreshBrowserTime: 5, // 仪表盘刷新时间 默认5分钟
  refreshViewEnable: false, // 开启图表刷新（默认关闭）
  refreshViewLoading: true, // 仪表盘图表loading提示
  refreshUnit: 'minute', // 仪表盘刷新时间带外 默认 分钟
  refreshTime: 5, // 仪表盘刷新时间 默认5分钟
  ...
  dashboardAdaptor: 'keepHeightAndWidth', //仪表盘预览展示适配方式 keepHeightAndWidth=高度宽度独立缩放(默认模式)，withWidth=跟随宽度
  scale: 60,
  scaleWidth: 60,
  scaleHeight: 60,
